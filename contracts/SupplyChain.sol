...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address,"msg.sender must be the owner of the contract");
    _;
  }

  modifier paidEnough(uint _price) {
    require(msg.value >= _price,"msg.value must be greater or equal price");
    _;
  }
  ...
  modifier forSale(uint _sku) {
    require((items[_sku].state == State.ForSale) && (skuCount > 0),"ForSale state required!");
    _;
  }
  modifier sold(uint _sku) {
    require(items[_sku].state == State.Sold,"Sold state required!");
    _;
  }
  modifier shipped(uint _sku) {
    require(items[_sku].state == State.Shipped,"Shipped state required!");
    _;
  }
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received,"Received state required!");
    _;
  }
